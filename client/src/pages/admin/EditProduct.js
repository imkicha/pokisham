  ...
  const fetchProduct = async () => {
    try {
      ...
      if (data.success) {
        ...
        if (product.bookingConfig) {
          setBookingConfig({
            commissionPercentage: product.bookingConfig.commissionPercentage ?? 10,
            minQuantity: product.bookingConfig.minQuantity ?? 1,
            maxQuantity: product.bookingConfig.maxQuantity ?? 100,
            leadTimeDays: product.bookingConfig.leadTimeDays ?? 2,
            availableCities: (product.bookingConfig.availableCities || []).join(', '),
          });
        }
      }
    } catch (error) {
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6 max-w-3xl">
        <div className="space-y-6">
          ...
          {/* Stock and SKU - hidden for booking products */}
          {productType !== 'booking' && (
            <div className="grid grid-cols-2 gap-4">
              <div>
